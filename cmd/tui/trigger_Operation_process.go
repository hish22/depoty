...
func TriggerInstallProcess(key tcell.Key, app *tview.Application, foundPkgsTable *tview.Table, InstallConfModal *tview.Modal) {
...
func TriggerUpdAndDelProcess(key []tcell.Key, app *tview.Application, PkgsTable *tview.Table, UpdateConfModal *tview.Modal, DeleteModal *tview.Modal) {
	// Handle the Operation button & press
	PkgsTable.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		// Start the Update process if CTRL + U pressed
		if event.Key() == key[0] {
			// No Updation if found packages is empty
			...
			// No Deletion if found packages is empty
			if PkgsTable.GetRowCount() != 0 {
				// Show Confirmation box
